...
```
Our model has an accuracy of `r round(accuracy*100,2)`%. We could have gotten this number from the confusion matrix results

The out-of-sample error is equal to the complimentary of this number, i.e. `r round(1-accuracy,2)`
...
So for our model, the out-of-sample error is equal to `r round((1-accuracy)*100,2)`%. 
...
## Conclusion
We used random forests algorithm to predict the quality of perfomance of athletes. Our model had an accuracy of `r round(accuracy*100,2)`% and an out-of-sample error of `r round((1-accuracy)*100,2)`%. After applying our model to the testing dataset and after the submission of the results to the Coursera servers we got all of the predictions correctly.